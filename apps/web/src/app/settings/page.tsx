...
export default function SettingsPage() {
    ...
    async function fetchConfig() {
      ...
      if (data) {
        data.forEach((item: UserConfig) => {
          if (item.key === "target_milk_ml")
            setMilkTarget(parseFloat(item.value));
          if (item.key === "target_sleep_hours")
            setSleepTarget(parseFloat(item.value));
  ...
  const handleMigrate = async () => {
    ...
    try {
      ...
      toast.success(
        `成功迁移 ${actCount} 条活动记录和 ${confCount} 条配置信息！`,
      );
    } catch (err) {
      console.error(err);
      toast.error(
        "迁移失败：请确保数据库已添加 user_id 列，并暂时关闭了 RLS。",
      );
    } finally {
      setMigrating(false);
    }
        ...
        [
          {
            user_id: user.id,
            key: "target_milk_ml",
            value: milkTarget.toString(),
          },
          {
            user_id: user.id,
            key: "target_sleep_hours",
            value: sleepTarget.toString(),
          },
        ...
        <Card className="border-none shadow-2xl bg-card/60 backdrop-blur-xl">
          <CardHeader>
            <CardTitle className="text-xl font-semibold">
              {t("settings.daily_standards")}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="milk" className="text-sm font-medium opacity-80">
                {t("settings.milk_target")}
              </Label>
              ...
              <p className="text-xs text-muted-foreground italic">
                {t("settings.milk_standard")}
              </p>
            </div>

            <div className="space-y-2">
              <Label htmlFor="sleep" className="text-sm font-medium opacity-80">
                {t("settings.sleep_target")}
              </Label>
              <div className="flex gap-4 items-center">
                <Input
                  id="sleep"
              ...
              <div className="flex p-1 bg-background/50 rounded-xl gap-1 border border-muted">
                {[
                  ...
                  <button
                    ...
                    className={`
                      flex-1 py-3 px-4 min-h-[44px] rounded-lg text-sm font-medium transition-all
                      ${
                        currentTheme === t.id
                          ? "bg-primary text-primary-foreground shadow-md"
                          : "text-muted-foreground hover:bg-background/80"
