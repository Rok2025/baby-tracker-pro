...
### 2.4 分享功能 (Sharing)
- **分享给微信好友**：在首页点击右上角「...」菜单，选择「转发」即可将小程序分享给微信好友。
- **分享到朋友圈**：在首页点击右上角「...」菜单，选择「分享到朋友圈」将小程序分享至您的朋友圈（需微信 7.0.12+ 版本）。
- **温馨文案**：分享时自动配置吸引人的中文文案，让更多家庭了解这款宝宝成长助手。
...
### 3.6 分享小程序
1. **分享给微信好友**：在首页点击右上角「...」按钮，选择「转发」，即可将小程序分享给微信好友或微信群。
2. **分享到朋友圈**：在首页点击右上角「...」按钮，选择「分享到朋友圈」，让您的朋友圈好友也能使用这款工具。
3. 分享后，您的好友点击分享卡片即可直接打开小程序首页。
...
## 6. 问题修复记录 (v1.3.1 - 2026-01)
### 6.1 UI & 交互修复
- **侧滑菜单空隙修复**：解决了 SwipeAction 组件在不同机型上出现白色空隙的问题。
  - *原因*：JS 逻辑像素与 CSS rpx 单位转换不匹配。
  - *方案*：统一使用 rpx 逻辑，JS 动态计算宽度的 `280rpx` 等效值。
- **样式升级**：优化了编辑/删除按钮的视觉风格（字体加粗、字号增大），使其符合原生 iOS/WeChat 设计规范。
- **布局修正**：修复了列表项 `margin` 导致的布局高度计算错误。

### 6.2 数据操作核心修复 (Supabase Client)
- **重构客户端内核**：
  - 将 `supabase.ts` 重构为标准的 **Builder 模式**，支持链式调用（如 `.update({...}).eq('id', ...)`），解决了之前的类型错误。
- **修复 "修改变新增" Bug**：
  - *问题*：旧版客户端通过 `POST` 伪装 `PATCH` 请求（Tunneling），导致服务器识别错误，将更新操作误判为插入新记录。
  - *方案*：**移除伪装逻辑**，全面启用微信小程序原生的 `PATCH` 和 `DELETE` 请求方法。
- **修复删除失败 (400 Error)**：
  - *问题*：伪装删除请求时 Body 为空导致格式校验失败。
  - *方案*：切换为原生 `DELETE` 方法，彻底解决此问题。
